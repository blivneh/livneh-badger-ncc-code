...
for(s in 1:nsitenames){
	...
	min.lon = huc.lon[min(chosen.area[,1]) - buffer] - 360
	max.lon = huc.lon[max(chosen.area[,1]) + buffer] - 360
	min.lat = huc.lat[min(chosen.area[,2]) - buffer]
	max.lat = huc.lat[max(chosen.area[,2]) + buffer]

	basin.area = sum(cos(pi* huc.lat[chosen.area[,2]]/180)*((110.5*1000/8))^2)
	...
	for(m in 1:nmodels){
		...
		flow.values = flow.values * basin.area * 0.001 * 0.000810708486253 / dim(chosen.area)[1]
